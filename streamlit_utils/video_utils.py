...
class Video:
    def __init__(self, 
                 video: UploadedFile, 
                 video_root: str = SAVE_VIDEO, 
                 get_fps: int = 10):
        """Класс видеоданных.

        Args:
            video (UploadedFile): загруженные данные из Streamlit
            video_root (str, optional): путь до директории сохранения логов и результатов детекции по видео. Defaults to SAVE_VIDEO.
    ...
    @staticmethod
    def get_timestamps(classes: List[int], 
                       nframes: int) -> Dict[str, int]:
        """Получение ключевых точек таймлайна, где обнаружены интересующие детекции. Трекинг не предусмотрен, поэтому за новую точку принимается та, до которой не было обноружено объектов такого типа. Типы обобщены на Опасные и Прочие.

        Args:
            classes (List[int]): лист длинной == числу проанализированных кадров, состоящий из 0, -1, 1, где 1 - опасность, 0 - прочее, -1 - на кадре нет (интересующих) детекций
            nframes (int): число кадров в секунду для финального видео

        Returns:
            Dict[str, int]: словарь вида тег_индекс: секунда на видео, где тег - опасность или прочее, а индекс соответствует индексу фрагмента 
        """
        init_tags = []
    ...
    def make_video(self, 
                   select_classes: List[int]) -> Dict[str, int]:
        """Создание видео с отрисовкой ббоксов на сохраненных фото. Данный метод не запускает инференс модели, а использует сохраненные в файлах предсказания.
        ...
        video = cv2.VideoWriter(self.tmp_path, fourcc, self.get_fps, (res_width, res_height), True)
        timeline = []
