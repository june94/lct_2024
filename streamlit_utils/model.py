...
class Model:
    ...
    @staticmethod
    def voc_to_yolo(x1: float, 
                    y1: float, 
                    x2: float, 
                    y2: float, 
                    image_w: int, 
                    image_h: int) -> List[float]:
    ...
    @staticmethod
    def save_preds(result: Any, #Union[PredictionResult, Results], 
                   image_height: int, 
                   image_width: int, 
                   image_name: str, 
                   save_path: str, 
        ...
        Args:
            ...
            save_thresh (Optional[List], optional): лист длинной == кол-во классов с порогами уверенности детекции для каждого из классов, детекции ниже этого порога в документ не сохраняются. Defaults to None.
        """
    ...
    @staticmethod
    def plot_preds(image: np.ndarray, 
                   bboxes: List[List],
                   classes: List[str],
...
class CustomYolov8DetectionModel(Yolov8DetectionModel):
    """Обертка Yolov8 модели из SAHI для возможности использования onnx и tensorrt инференса с батчом размером > 1.
    """
    def load_model(self):
        """Загрузка претрена.
        """
