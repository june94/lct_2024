...
def main():
    ...
    with photo_tab:
        ...
        photo_input = input_container.file_uploader(
            "**Загрузить фото или архив:**",
        ...
        if len(photo_input) > 0:
            if input_container.button("Начать расчёт", key="photo_upload"):
                ...
                if photo_input[0].name.endswith(".tar"):
                    try:
                        ...
                        input_container.error(
                            f"Проблема с архивом {photo_input[0].name}"
                        )
                ...
                with input_container.status(
                    "Производится расчёт", expanded=True
                ) as status:
                    avg_time = 0
                    ...
                    for c, file in enumerate(proc_input):
                        ...
                        if c % IMG_BATCH_SIZE == 0 or c == (len(proc_input) - 1):
                            ...
                            for huge_img, meta in zip(huge_imgs, huge_meta):
                                x_ratio, y_ratio, slice_width, slice_height = (
                                    get_auto_slice_params(height=meta[0], width=meta[1])
                                )
                                sahi_results.append(
                                    model(
                                        huge_img,
                    ...
                    status.update(
                        label=f"Расчёт окончен. Предсказания сохранены в {os.path.abspath(SAVE_IMAGES_TXT)}. Время инференса модели {avg_time} fps.",
                        state="complete",
                        expanded=False,
                    )
    ...
    with video_tab:
        ...
        video_input = input_container.file_uploader(
            "**Загрузить видео:**", type=["mp4", "mkv", "mov", "avi"]
        )
        ...
        if video_input is not None:
            ...
            if input_container.button("Начать расчёт", key="video_upload"):
                ...
                with input_container.status("Работа с видео", expanded=True) as status:
                    ...
                    if len(os.listdir(video.labels_path)) == len(video):
                        ...
                        video_batch_size = (
                            VIDEO_BATCH_SIZE + 1 if sahi_condition else VIDEO_BATCH_SIZE
                        )
                        if sahi_condition:
                            x_ratio, y_ratio, slice_width, slice_height = (
                        ...
                        for c, image_batch in enumerate(
                            Video.batch(image_dir, video_batch_size)
                        ):
                            full_image_paths = [
                                f"{video.init_images_path}/{image}"
                            ...
                            if len(yolo_paths):
                                yolo_results = model(
                                    yolo_paths, slice_infer=False
                                )  # [0]

                                for result, image_name in zip(
                                    yolo_results, image_names
                                ):
                                    avg_time += result.speed["inference"] / 1000
                                    model.save_preds(
                                        result,
                            ...
                            if sahi_condition:
                                ...
                                avg_time += sahi_result.durations_in_seconds[
                                    "prediction"
                                ]
                                model.save_preds(
                                    sahi_result,
                        ...
                        input_container.success(
                            f"Предсказания сохранены в {os.path.abspath(video.root_dir)}. Время инференса модели {avg_time} fps."
                        )

                    status.update(
                        label=f"3. Расчёт окончен", state="complete", expanded=False
                    )
            ...
            if st.session_state["video_button_clicked"]:
                ...
                select_classes = result_container.multiselect(
                    "Выбрать класс(ы) для отображения:",
                    CATEGORIES,
                ...
                if st.session_state["choice_button_clicked"]:
                    ...
                    if len(st.session_state["timestamps"]) > 0:
                        select_moment = result_container.selectbox(
                            "Выбрать интервал", st.session_state["timestamps"].keys()
                        )

                        st.session_state["slider_button_clicked"] = True

                        result_container.video(
                            st.session_state["video_output"],
                            start_time=st.session_state["timestamps"][select_moment],
                            autoplay=False,
                            loop=False,
                        )
                    else:
                        ...
                        result_container.video(
                            st.session_state["video_output"], autoplay=True, loop=False
                        )
